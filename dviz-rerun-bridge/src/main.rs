//! # DViz Dora Bridge - Universal Zenoh Publisher
//!
//! A generic Dora node that publishes ANY sensor data via Zenoh using
//! the DViz universal message protocol.
//!
//! ## Architecture
//! - **Robot side**: This bridge receives Dora inputs, publishes to Zenoh
//! - **PC side**: dviz-shell subscribes to Zenoh, displays in Rerun (generic)
//!
//! ## Universal Protocol
...
impl GraphState {
    ...
    fn record_input(&mut self, input_id: &str, timestamp: f64) {
        // Always mark dviz_bridge as active
        self.node_activity.insert("dviz_bridge".to_string(), timestamp);
...
async fn main() -> Result<()> {
    env_logger::init();
    log::info!("Starting DViz Universal Dora Bridge");

    // Get configuration from environment
    let topic_prefix = env::var("ZENOH_TOPIC_PREFIX").unwrap_or_else(|_| "dviz".to_string());
    ...
    publish_log(&session, &topic_prefix, 0.0, "INFO", "DViz bridge started, waiting for data...", "dviz_bridge").await;
        ...
        match event {
            Event::Input { id, metadata: _, data } => {
                ...
                *count += 1;
                ...
                // Publish I/O activity for dviz_bridge inputs (showing live message data)
            ...
            Event::Stop(_) => {
                ...
                publish_log(&session, &topic_prefix, timestamp, "INFO", &msg, "dviz_bridge").await;
                break;
            }
    ...
    log::info!("DViz Dora Bridge stopped, published {} total frames", frame_count);
    Ok(())
}
